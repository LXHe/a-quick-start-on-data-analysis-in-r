...
有时我们也会碰到在字符串的前后有空格的情况，这时我们可以用`[\\s]*`去进行正则表达式匹配或者使用***stringr***包的`str_trim()`函数移除位于字符串两端的空格。如果需要移除字符串中间的多余空格（即仅保留一个空格），可以使用`str_squish()`函数。
...
注意与`ifelse()`函数\@ref(chapter1-label-ifelse)的区别。
```{r chapter1-if-else}
# if(){}else{}只接受一个逻辑值
