...
bmi[bmi %in% c(24:30)] # 筛选BMI在[24,30]范围内的元素
bmi[(bmi>=24)&(bmi<=30)] # 筛选BMI在[24,30]范围内的元素
```
***注意***：当向量中有缺失值 `NA` 时，上述的逻辑运算中，除了 `%in%` ，其余运算都会将缺失值囊括进来。
...
```
采用`dim()`函数获取矩阵的维度，`nrow()`函数获取矩阵的行数，`ncol()`函数获取矩阵的列数。
...
```
`list()`元素的获取采用`[[]]`+索引值的方式，添加元素使用`append()`函数。
